...
<div class="container-fluid">
			...
			<div class="page-header">
				<div class="page-title">
					<ol class="breadcrumb ">
						...
						<li class="active">{!! __('backendcp::contentcategory.breadcrum.edit') !!}</li>
					</ol>
				</div>
			</div>
		</div>
	...
	<section id="main-content">
		<div class="row">
			<form name="adminForm" class="form-horizontal" action="{{route('admin.content.category.update')}}" method="post" enctype="multipart/form-data">
			{{ csrf_field()}}
				...
				<div class="card">
					...
					<div class="card">
							...
							<div class="form-group {{ $errors->first('title', 'has-error') }}">
								...
								<div class="col-sm-8">
									<input type="text" name="title" class="form-control" placeholder="{{ __('backendcp::contentcategory.fields.title_placeholder') }}" value="{{$category->title  or old('title')}}">
								</div>
							</div>
							...
							<div class="form-group" {{ $errors->first('fulltext', 'has-error') }}>
								...
								<div class="col-sm-8">
									<textarea name="description" placeholder="{{ __('backendcp::contentcategory.fields.fulltext_placeholder') }}" class="textarea form-control">{{ $category->description or old('description')}}</textarea>									
								</div>
							</div>
                                ...
                                <div class="col-sm-8">
                                    <div class="fileinput fileinput-new" data-provides="fileinput">
                                        <div class="fileinput-new thumbnail" style="width: 150px; height: 150px;">
                                            @if($category->image)
                                               <img src="{{asset('uploads/contentcategory/'.$category->image)}}">
                                            @else
						...
						<div class="basic-form m-t-20">
							<div class="form-group">
								...
								<select class="form-control border-none input-flat bg-ash" name="status">
									...
									<option value="1" @if( 1 == $category->status ) selected @endif >{!! __('backendcp::contentcategory.fields.publish') !!}</option>
									<option value="0" @if( 0 == $category->status ) selected @endif >{!! __('backendcp::contentcategory.fields.unpublish') !!}</option>
								</select>
						...
						<div class="basic-form m-t-20">
							<div class="form-group">
								...
								<select class="form-control border-none input-flat bg-ash" name="featured">
									<option value="0" @if( 1 == $category->featured ) selected @endif >{!! __('backendcp::contentcategory.fields.no') !!}</option>
									<option value="1" @if( 0 == $category->featured ) selected @endif>{!! __('backendcp::contentcategory.fields.yes') !!}</option>
