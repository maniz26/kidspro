...
<div class="container-fluid">
			...
			<div class="page-header">
				<div class="page-title">
					<ol class="breadcrumb ">
						...
						<li class="active">{!! __('backendcp::content.breadcrum.edit') !!}</li>
					</ol>
				</div>
			</div>
		</div>
	...
	<section id="main-content">
		<div class="row">
			<form name="adminForm" class="form-horizontal" action="{{route('admin.content.update')}}" method="post" enctype="multipart/form-data">
			{{ csrf_field()}}
				...
				<div class="card">
					...
					<div class="card">
							...
							<div class="form-group {{ $errors->first('title', 'has-error') }}">
								...
								<div class="col-sm-8">
									<input type="text" name="title" class="form-control" placeholder="{{ __('backendcp::content.fields.title_placeholder') }}" value="{{$content->title  or old('title')}}">
								</div>
							</div>
							<div class="form-group">
								...
								<div class="col-sm-8">
									<textarea name="introtext" placeholder="{{ __('backendcp::content.fields.introtext_placeholder') }}" class="form-control">{{ $content->introtext or old('introtext') }}</textarea>									
								</div>
							</div>
							...
							<div class="form-group" {{ $errors->first('fulltext', 'has-error') }}>
								...
								<div class="col-sm-8">
									<textarea name="fulltext" placeholder="{{ __('backendcp::content.fields.fulltext_placeholder') }}" class="textarea form-control">{{ $content->fulltext or old('fulltext') }}</textarea>									
								</div>
							</div>
                                ...
                                <div class="col-sm-8">
                                    <div class="fileinput fileinput-new" data-provides="fileinput">
                                        <div class="fileinput-new thumbnail" style="width: 150px; height: 150px;">
                                            @if($content->image)
                                               <img src="{{asset('uploads/contents/'.$content->image)}}">
                                            @else
						...
						<div class="basic-form m-t-20">
							<div class="form-group">
								...
								<select class="form-control border-none input-flat bg-ash" name="category_id">
									...
									<option value="{{$category->id}}" @if( $category->id == $content->category_id ) selected @endif >{{$category->title}}</option>
									@endforeach
						...
						<div class="basic-form m-t-20">
							<div class="form-group">
								...
								<select class="form-control border-none input-flat bg-ash" name="status">
									...
									<option value="1" @if( 1 == $content->status ) selected @endif >{!! __('backendcp::content.fields.publish') !!}</option>
									<option value="0" @if( 0 == $content->status ) selected @endif >{!! __('backendcp::content.fields.unpublish') !!}</option>
								</select>
						...
						<div class="basic-form m-t-20">
							<div class="form-group">
								...
								<select class="form-control border-none input-flat bg-ash" name="featured">
									<option value="0" @if( 1 == $content->featured ) selected @endif >{!! __('backendcp::content.fields.no') !!}</option>
									<option value="1" @if( 0 == $content->featured ) selected @endif>{!! __('backendcp::content.fields.yes') !!}</option>
